...
class AspireConSent(nn.Module):
    def __init__(self, hf_model_name, device=torch.device('cpu')):
        """
    ...
    def consent_reps_bert(self, bert_batch, batch_senttok_idxs, num_sents):
        ...
        tokid_tt, seg_tt, attnmask_tt = tokid_tt.to(self.device), seg_tt.to(self.device), attnmask_tt.to(self.device)
        ...
        for sent_i in range(max_sents):
            ...
            sent_mask = sent_mask.to(self.device)
...
class AllPairMaskedWasserstein:
    def __init__(self, model_hparams, device=torch.device("cpu")):
